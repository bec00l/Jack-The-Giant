//
//  CloudController.swift
//  Jack the Giant
//
//  Created by David Hurd on 8/21/17.
//  Copyright © 2017 Imagitale Studios. All rights reserved.
//

import SpriteKit

class CloudController {
    var lastCloudPositionY = CGFloat()
    let cloudHeightOffset : CGFloat =  6.0
    let cloudWidthOffset : CGFloat =  5.0
    let collectableController = CollectablesController()
    
    func shuffle( cloudsArray : [SKSpriteNode]) -> [SKSpriteNode]{
        var cloudsArray = cloudsArray
        for i in ((0 + 1)...cloudsArray.count - 1).reversed() {
            let j = Int(arc4random_uniform(UInt32(i - 1)))
            
            swap(&cloudsArray[i], &cloudsArray[j])
        }
         return cloudsArray
    }
    
    func createClouds() -> [SKSpriteNode] {
        var clouds = [SKSpriteNode]()
        
        for var i in 0 ..< 2 {
           let cloud1 = SKSpriteNode(imageNamed: "Cloud 1")
            cloud1.name = "1"
           let cloud2 = SKSpriteNode(imageNamed: "Cloud 2")
            cloud2.name = "2"
           let cloud3 = SKSpriteNode(imageNamed: "Cloud 3")
            cloud3.name = "3"
           let darkCloud = SKSpriteNode(imageNamed: "Dark Cloud")
            darkCloud.name = "Dark Cloud"
            
            cloud1.yScale = 0.9
            cloud1.xScale = 0.9
            
            cloud2.yScale = 0.9
            cloud2.xScale = 0.9
            
            cloud3.yScale = 0.9
            cloud3.xScale = 0.9
            
            darkCloud.yScale = 0.9
            darkCloud.xScale = 0.9
            
            cloud1.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: cloud1.size.width - cloudWidthOffset, height: cloud1.size.height - cloudHeightOffset))
            cloud1.physicsBody?.affectedByGravity = false
            cloud1.physicsBody?.restitution = 0
            cloud1.physicsBody?.collisionBitMask = ColliderType.Player
            cloud1.physicsBody?.contactTestBitMask = ColliderType.Cloud
            
            cloud2.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: cloud2.size.width - cloudWidthOffset, height: cloud2.size.height - cloudHeightOffset))
            cloud2.physicsBody?.affectedByGravity = false
            cloud2.physicsBody?.restitution = 0
            cloud2.physicsBody?.collisionBitMask = ColliderType.Player
            cloud2.physicsBody?.contactTestBitMask = ColliderType.Cloud
            
            cloud3.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: cloud3.size.width - cloudWidthOffset, height: cloud3.size.height - cloudHeightOffset))
            cloud3.physicsBody?.affectedByGravity = false
            cloud3.physicsBody?.restitution = 0
            cloud3.physicsBody?.collisionBitMask = ColliderType.Player
            cloud3.physicsBody?.contactTestBitMask = ColliderType.Cloud
            
            darkCloud.physicsBody = SKPhysicsBody(rectangleOf: darkCloud.size)
            darkCloud.physicsBody?.affectedByGravity = false
            darkCloud.physicsBody?.collisionBitMask = ColliderType.Player
            darkCloud.physicsBody?.contactTestBitMask = ColliderType.DarkCloudAndCollectibles
            
            clouds.append(cloud1)
            clouds.append(cloud2)
            clouds.append(cloud3)
            clouds.append(darkCloud)
        }
        clouds = shuffle(cloudsArray: clouds)
        return clouds
    }
    
    func arrangeCloudsInScene( scence : SKScene,  distanceBetweenClouds: CGFloat,  center : CGFloat,  minX : CGFloat,  maxX : CGFloat, player: Player, initialClouds : Bool){
        
        var clouds = createClouds()
        
        if initialClouds {
            while clouds[0].name == "Dark Cloud" {
                //shuffle clouds
                clouds = shuffle(cloudsArray: clouds)
                
            }
        }
        
        var positionY = CGFloat()
        
        if initialClouds {
            positionY = center - 100
        } else {
            positionY = lastCloudPositionY
        }
        
        var random = 0
        
        
        for i in 0...clouds.count - 1 {
            var randomX = CGFloat()
            if random == 0{
                randomX = randomBetweenNumbers(firstNumber: center + 90, secondNumber: maxX)
                random = 1
            } else if random == 1{
                randomX = randomBetweenNumbers(firstNumber: center - 90, secondNumber: minX)
                random = 0
                
            }
            clouds[i].position = CGPoint(x : randomX , y: positionY)
            clouds[i].zPosition = 3
            
            if !initialClouds {
                if (Int(randomBetweenNumbers(firstNumber: 0, secondNumber: 7)) >= 3){
                    if clouds[i].name != "Dark Cloud" {
                        let collectable = collectableController.getCollectables()
                        collectable.position = CGPoint(x : clouds[i].position.x, y: clouds[i].position.y + 60)
                        scence.addChild(collectable)
                    }
                    
                }
            }
            scence.addChild(clouds[i])
            positionY -= distanceBetweenClouds
            lastCloudPositionY = positionY
        }
        
        if initialClouds {
            player.position = CGPoint(x: clouds[0].position.x, y: clouds[0].position.y + 9)
        }
    }
    
    func randomBetweenNumbers (firstNumber : CGFloat, secondNumber : CGFloat) -> CGFloat {
        return CGFloat(arc4random()) / CGFloat(UINT32_MAX) * abs(firstNumber - secondNumber) + min(firstNumber, secondNumber)
    }
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
}
